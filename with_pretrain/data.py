...
def prepare_pretrain_data(images_x, seqs_y, dictLen):

    heights_x = [s.shape[1] for s in images_x]
    widths_x = [s.shape[2] for s in images_x]
    lengths_y_validation = [len(s)==dictLen for s in seqs_y]

    if False in lengths_y_validation: raise Exception("y label not uniform")
